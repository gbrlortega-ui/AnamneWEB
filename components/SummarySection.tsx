  ...
  return (
    <div id="print-root-container" className="p-0 bg-transparent">
      <style dangerouslySetInnerHTML={{ __html: `
        /* RESET GLOBAL PARA IMPRESSÃO - CORREÇÃO DE CORTE */
        @media print {
          /* Força todos os elementos pais a permitirem overflow e altura automática */
          html, body, #root, [class*="h-screen"], [class*="overflow-hidden"], main, article, section {
      ...
      <div id="clinical-document" className="max-w-[850px] mx-auto bg-white p-4 md:p-10 font-sans print:max-w-none print:p-0">
        ...
        <header className="mb-8 border-b-2 border-black pb-4 flex justify-between items-end">
          <div>
            <h1 className="text-2xl font-black text-black uppercase tracking-tight">Registro Médico de Atendimento</h1>
        ...
        <section className="record-section border border-slate-200 p-6 rounded-2xl bg-slate-50/20">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-8 text-[11px] id-grid">
            <div className="col-span-2 md:col-span-3 border-b border-slate-100 pb-2">
              <span className="font-bold text-slate-400 block uppercase text-[8px]">Nome do Paciente</span>
            ...
            <div>
              <span className="font-bold text-slate-400 block uppercase text-[8px]">Profissão / Ocupação</span>
            ...
            {record.id.cuidador && (
              <div>
                <span className="font-bold text-slate-400 block uppercase text-[8px]">Cuidador Principal</span>
                <span className="font-bold text-slate-800">{record.id.cuidador}</span>
              </div>
            )}
            {record.id.responsavel && (
              <div className="col-span-2">
                <span className="font-bold text-slate-400 block uppercase text-[8px]">Responsável Legal</span>
        ...
        <section className="record-section">
          ...
          {qpHma ? (
            ...
            <p className="text-xs italic text-slate-400">Nenhum registro de História Clínica preenchido.</p>
          )}
        </section>
        ...
        <section className="record-section">
          ...
          {(cvRiskText || ivcfResultText) && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
              {cvRiskText && (
                <div className={`p-5 border-2 rounded-xl risk-block ${getCVColorClasses(cvRiskLevel)} shadow-sm`}>
                  <span className="text-[8px] font-black uppercase block opacity-60 mb-1">Risco Cardiovascular Global (OPAS/WHO)</span>
                  <p className="text-xs font-black uppercase tracking-tight">{cvRiskText}</p>
                </div>
              )}
              {ivcfResultText && (
                <div className={`p-5 border-2 rounded-xl risk-block ${getIVCFColorClasses(ivcfLevel)} shadow-sm`}>
                  <span className="text-[8px] font-black uppercase block opacity-60 mb-1">Avaliação IVCF-20 (Escore: {ivcfScore})</span>
                  <p className="text-xs font-black uppercase tracking-tight">{ivcfResultText}</p>
                </div>
              )}
            </div>
          )}
        ...
        <section className="record-section">
          ...
          <div className="grid grid-cols-3 md:grid-cols-6 lg:grid-cols-9 gap-2 mb-6 vitals-grid-print">
            {[
              ...
              <div key={i} className="vital-box-print rounded-xl flex flex-col justify-center">
                <span className="text-[7px] font-black text-slate-400 uppercase block mb-1 tracking-widest">{v.label}</span>
                <span className="text-[11px] font-black text-slate-900 block">{v.value}</span>
                {v.status && <span className={`text-[7px] font-black uppercase mt-1 block leading-tight ${v.status.color}`}>{v.status.label}</span>}
              </div>
            ))}
          </div>
          ...
          <div className="space-y-4">
            {Object.entries(record.exameFisico).map(([key, val]) => {
              ...
              return (
                <div key={key} className="border-b border-slate-50 pb-2">
                  <span className="text-[8px] font-black text-slate-400 uppercase block mb-1">{labelMap[key] || key}</span>
            ...
            {record.soap?.o && (
              <div className="border-b border-slate-100 pb-2 pt-2">
                <span className="text-[8px] font-black text-primary uppercase block mb-1">Objetivo / Exames Complementares (SOAP)</span>
                <div className="text-[11px] text-slate-800 text-content">{record.soap.o}</div>
              </div>
            )}
          </div>
        </section>
        ...
        {/* SÍNTESE E CONDUTA */}
        <section className="record-section">
          <h3>V. Raciocínio Clínico e Conduta</h3>
          <div className="space-y-6">
            {record.hipoteseDiagnostica && (
              <div>
                <span className="text-[9px] font-black text-slate-400 uppercase block mb-1">Hipótese Diagnóstica</span>
                <p className="text-sm font-black text-slate-900 leading-tight">{record.hipoteseDiagnostica}</p>
              </div>
            )}
            
            {(record.fatoresRisco || (record.soap?.assessments && record.soap.assessments.length > 0)) && (
              <div>
                <span className="text-[9px] font-black text-slate-400 uppercase block mb-1">Avaliação e Fatores de Risco</span>
                {record.fatoresRisco && <div className="text-content italic text-slate-600 text-sm mb-2">{record.fatoresRisco}</div>}
            ...
            {(record.conduta || (record.soap?.plans && record.soap.plans.length > 0)) && (
              <div className="p-6 bg-slate-50 border-2 border-slate-200 rounded-3xl">
                <span className="text-[9px] font-black text-primary uppercase block mb-2 tracking-widest">Plano Terapêutico e Orientações</span>
                {record.conduta && <div className="text-content font-bold text-slate-900 text-sm mb-4 leading-relaxed">{record.conduta}</div>}
                
                {record.soap?.plans && record.soap.plans.map((p, i) => p.text && (
                  <div key={i} className="text-[11px] text-slate-900 mb-2 last:mb-0 border-l-2 border-primary/20 pl-3">
        ...
        {/* ASSINATURAS NO FIM DO DOCUMENTO */}
        <div className="mt-20 pt-10 border-t border-slate-200">
          <div className="flex justify-around text-center">
            <div className="w-[45%] border-t border-black pt-4">
            ...
            <div className="w-[45%] border-t border-slate-300 pt-4">
              <div className="h-10"></div>
              <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mt-1">Carimbo e CRM do Médico Preceptor</p>
            </div>
          </div>
        </div>
